...
import { useAlphaDataStore, type Alpha } from '../DataManager';
...
const AlphaTable: React.FC = () => {
  const { alpha, connect, disconnect } = useAlphaDataStore();
  ...
  return (
    <div className="mx-2 lg:mx-3 overflow-hidden rounded-lg border">
      <div className="mx-2 my-2 flex flex-row gap-2">
        <StreamControls className="flex-grow-1 flex flex-row items-center" data={alpha.length > 0 ? alpha[0] : null} />
...
const StreamControls = ({ className = "", data }: StreamControlsProps) => {
  ...
  const { status, reconnect, disconnect } = useAlphaDataStore();
  useEffect(() => {
    if (data) {
      if (data.at > 0) {
        setLatency(Date.now() - data.at)
      } else if (data.bt > 0) {
        setLatency(Date.now() - data.bt)
      }
    }
  }, [data])
  useEffect(() => {
